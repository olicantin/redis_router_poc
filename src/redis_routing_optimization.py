...
from customer_baseline import format_query, classify, COST_IN, COST_OUT
...
COST_IN, COST_OUT = 0.09/1_000_000, 0.29/1_000_000 #HF costs with nscale
...
# If SSL is enabled on the endpoint, use rediss:// as the URL prefixcccccbuletdirvjrctcbgbnkvucujguvugckkjtkuten
...
def build_routes(dataset: dict) -> list:
    ...
    for category, articles in dataset.items():
        route = Route(
            name=category,
            references=[tarticlet for _,tarticlet in articles],
...
def route_with_llm_fallback_queries_with_stats(queries: list):
    ...
    for query in queries:
        ...
        if match:
            ...
            responses.append((query['ArticleId'], match.name))  # keep index alignment with queries
            pred_label = getattr(match, "name", "") or ""
        ...
        try:
            if response and response.choices:
                ...
                if json.loads(content).get("label", "") in LABELS:
                    accurate += 1
        except Exception:
    ...
    exec_time = (sum(times) / len(times)) if times else 1
    accuracy = (accurate / len(queries)) if queries else 1
...
def main() -> int:
    ...
    test_dataset = load_dataset("bbc-news-articles-labeled/BBC News Test.csv")[0:5]
